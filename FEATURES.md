...
| **Model scorecard** | Per driving model, from your trips and takeovers: km, engaged % (weighted by drive time), disengagements and takeovers per 100 km, too-weak/too-strong split. Ranked by takeovers/100 km then disengagements; a model needs 20 km before it is ranked | from `sp_trips.csv` + `sp_overrides.csv` |
| **Takeover map** | GPS on every takeover and disengagement, plus a sparse drive track (one point per 15 s, last 30 drives). Phone map (Leaflet + OpenStreetMap; plain plot when the phone is offline) fits your drive on load and keeps your zoom across refreshes | `sp_overrides.csv`, `sp_disengagements.csv`, `sp_track.csv` |
| **Auto-save clips** | Queues a clip (saved 20 s later so it includes the aftermath) after: hard braking ≤ −4 m/s² above 20 km/h; a hard takeover ≥ 1.5 m/s² curve above 50 km/h; or a disengagement above 40 km/h with no pedal, wheel, or cruise-button input. Min 60 s apart, max 20 a day, optional notification | `sunnypilot_clips.json` |
